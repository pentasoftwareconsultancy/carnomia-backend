...
export const registerUser = async (req, res) => {
  try {
    ...
    if (mobile) {
      const existingMobile = await User.findOne({ mobile });
      if (existingMobile) return res.status(400).json({ error: "Mobile already exists" });
    }
...
export const loginUser = async (req, res) => {
  ...
  try {
    ...
    if (email && password) {
      user = await User.findOne({ email }).select("+password +name");
      if (!user) return res.status(404).json({ message: "User not registered" });
      if (!user.password) return res.status(400).json({ message: "This account has no password set" });

      if (["admin", "engineer", "superadmin"].includes(user.role) && !email.endsWith("@carnomia.com")) {
        return res.status(400).json({ message: "Admins/Engineers/SuperAdmins must login with carnomia.com email" });
      }
    ...
    // Case 2: Mobile + Password login
    if (mobile && password) {
      if (!/^\d{10}$/.test(mobile)) return res.status(400).json({ message: "Invalid mobile number" });

      user = await User.findOne({ mobile }).select("+password +name");
      if (!user) return res.status(404).json({ message: "User not registered" });
      if (!user.password) return res.status(400).json({ message: "This account has no password set" });
    ...
    if (!user) return res.status(400).json({ message: "Please provide email & password or mobile & password" });
...
export const getusers = async (req, res) => {
  try {
    const users = await User.find({ role: "customer" }).select("+name");
    if (!users || users.length === 0) return res.status(404).json({ success: false, message: "No users found" });
